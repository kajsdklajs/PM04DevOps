...
class RandomProgressApp:
    def __init__(self, root):
        ...
        # Максимальное значение для ProgressBar (будет меняться)
    ...
    def create_widgets(self):
        # Метка для отображения случайного числа
        ...
        # TrackBar (Scale в tkinter)
        self.trackbar = ttk.Scale(
            self.root,
            from_=10,  # минимальное значение
            to=200,    # максимальное значение
            orient='horizontal',
            command=self.update_trackbar
        )
        self.trackbar.set(100)  # начальное значение
    ...
    def update_trackbar(self, value):
        ...
        # Обновляем текущее значение ProgressBar относительно нового максимума
        if max_value > 0:
    ...
    def reset_progress(self):
        ...
        # Генерация случайного числа от 0 до max_value
